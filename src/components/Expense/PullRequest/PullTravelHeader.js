...
function PullTravelHeader({ onDataUpdate }) {
  ...
  // useEffect(()=>{
  //   if(requestid){
  //     getExpenseTravelRequest(requestid).then((res)=>{
  //       form.setFieldsValue({
  //                           requestName: res.responseData?.requestName,
  //                           requestPolicy: res.responseData?.requestPolicy,
  //                           purpose: res.responseData?.purpose,
  //                         });
  //                         setStartDate(new Date(res.responseData?.startDate));
  //                         setEndDate(new Date(res.responseData?.endDate));
  //     })
  //   }
    ...
    requestPolicy()
      .then((res) => {
        if (res.responseCode === 200) {
          setPolicy(res.data);
          if (requestid) {
            getExpenseTravelRequest(requestid).then((response) => {
              if (response.responseCode === 200) {
                if(response.responseData)
                {
                  ...
                  const inputStartDate = new Date(
                    response.responseData?.startDate
                  );
  
                  const year = inputStartDate.getFullYear();
                  const month = (inputStartDate.getMonth() + 1)
                    .toString()
                    .padStart(2, "0");
                  const day = inputStartDate
                    .getDate()
                    .toString()
                    .padStart(2, "0");
                  ...
                  const monthend = (inputEndDate.getMonth() + 1)
                    .toString()
                    .padStart(2, "0");
                  const dayend = inputEndDate
                    .getDate()
                    .toString()
                    .padStart(2, "0");
                  ...
                  sessionStorage.setItem(
                    "requestPolicy",
                    response.responseData?.requestPolicy
                  );
                  if (formattedStartDate && formattedEndDate) {
                    const currentDate = new Date(response.responseData?.startDate);
  
                    while (
                      currentDate <= new Date(response.responseData?.endDate)
                    ) {
  ...
  const Saved = (value) => {
    ...
    if (Object.values(body).some((value) => value !== null && value !== "")) {
      // dispatch(travelHeader(obj));
      expenseTravelRequest(body).then((res) => {
        if (res.responseCode === 200) {
            ...
            >
              ...
              >
                <DatePicker
                  ...
                  required
                  minDate={startDate} // Disable dates before start date or today
                  className="form-control"
          ...
          >
            ...
            >
              <TextArea
                disabled
                rows={4}
                placeholder="Please breif your Purpose"
              />
            </Form.Item>
          </div>
        </div>
...
export default PullTravelHeader;
