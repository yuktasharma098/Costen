...
function Buss({TripType,triggerParentEffect}) {
  ...
  const handleSave = () => {
    ...
    if (filteredData.length > 0) {
      ...
      postTransport("bus", obj,requestPolicy).then((res) => {
        if (res.responseCode === 200) {
          message.success("Bus Data Saved Succesfully");
      ...
      dispatch(bus(reduxobj));
    } else {
      message.error("Add some data for bus to save");
    }
  };

  useEffect(() => {
    if (Object.keys(busData).length > 0) {
      if (busData.trips.length > 0) {
        if (busData?.tripType === "multiCity") {
          if (busData.trips.length > 0) {
            const initialFormDataArray = busData.trips.map((obj) => ({
        ...
        if (busData?.tripType === "oneway") {
          setDepartureDateONeWay(new Date(busData.trips[0].departureDate));

          if (busData.trips[0]) {
            const initialFormDataArray = {
              departureDateoneway:
                new Date(busData.trips[0].departureDate) || null,
              estimateCostoneway: busData.trips[0].estimateCost || "",
              fromoneway: busData.trips[0].from || "",
              tooneway: busData.trips[0].to || "",
            };
        ...
        if (busData?.tripType === "round") {
          setDepartureDateroundway(new Date(busData.trips[0].departureDate));
          setDepartureDateroundway1(new Date(busData.trips[0].departureDate));
          if (busData.trips[0]) {
            const initialFormDataArray = {
              departureDateroundway0:
                new Date(busData.trips[0].departureDate) || null,
              estimateCostroundway0: busData.trips[0].estimateCost || "",
              fromroundway0: busData.trips[0].from || "",
              toroundway0: busData.trips[0].to || "",
              departureDateroundway1:
                new Date(busData.trips[1].departureDate) || null,
              estimateCostroundway1: busData.trips[1].estimateCost || "",
              fromroundway1: busData.trips[1].from || "",
              toroundway1: busData.trips[1].to || "",
            };
