...
function Trains({TripType,triggerParentEffect}) {
    ...
    if (triptype === "multiCity") {
      ...
      for (const key in onewayform) {
        if (Object.prototype.hasOwnProperty.call(onewayform, key)) {
          const newKey = key.replace("oneway", ""); 
  ...
  if (filteredData.length > 0) {
    ...
    postTransport("train", obj,requestPolicy).then((res) => {
      if (res.responseCode === 200) {
        message.success("Train Data Saved Succesfully");
   ...
   let reduxobj = {
    ...
    dispatch(train(reduxobj));
  } else {
    message.error("Add some data for train to save");
  }
  ...
  useEffect(() => {
    if(Object.keys(trainData).length>0){
      if(trainData.trips.length>0){
        if (trainData?.tripType === "multiCity") {
          if (trainData.trips.length > 0) {
            const initialFormDataArray = trainData.trips.map((obj) => ({
        ...
        if (trainData?.tripType === "oneway") {
          setDepartureDateONeWay(
            new Date(trainData.trips[0].departureDate)
          );
  
          if (trainData.trips[0]) {
            const initialFormDataArray = {
              departureDateoneway:
                new Date(trainData.trips[0].departureDate) || null,
              estimateCostoneway: trainData.trips[0].estimateCost || "",
              fromoneway: trainData.trips[0].from || "",
              tooneway: trainData.trips[0].to || "",
            };
          ...
          if (trainData.trips[0]) {
            const initialFormDataArray = {
              departureDateroundway0:
                new Date(trainData.trips[0].departureDate) || null,
              estimateCostroundway0: trainData.trips[0].estimateCost || "",
              fromroundway0: trainData.trips[0].from || "",
              toroundway0: trainData.trips[0].to || "",
              departureDateroundway1:
                new Date(trainData.trips[1].departureDate) || null,
              estimateCostroundway1: trainData.trips[1].estimateCost || "",
              fromroundway1: trainData.trips[1].from || "",
              toroundway1: trainData.trips[1].to || "",
            };
